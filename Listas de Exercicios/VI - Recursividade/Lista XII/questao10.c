...
int removerCaracteres(char str[], char ch, int i)
{
    ...
    else // Caso Geral
    {
        if (str[i] == ch)
        {
            ...
            // Substituindo os caracteres repetidos pelo próximo caracter
            while (str[j])
            {
                str[j] = str[j + 1];
                j++;
            ...
            // Incrementando e chamando novamente a função
            return 1 + removerCaracteres(str, ch, i);
        }
        else
        {
            // Chamando a função para o próximo caracter da string
